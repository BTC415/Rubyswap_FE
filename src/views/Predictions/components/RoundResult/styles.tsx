...
import { Box, Flex, FlexProps, Text } from '@twinkykms/rubyswap-uikit'
...
export const PrizePoolRow: React.FC<PrizePoolRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      ...
      <Text bold>{`${getPrizePoolAmount(totalAmount)} EGEM`}</Text>
    </Row>
  )
}
  ...
  return (
    <Row height="18px" {...props}>
      ...
      <Flex alignItems="center">
        ...
        <Text mx="4px">|</Text>
        <Text fontSize="12px" lineHeight="18px">{`${formatBnb(amount)} EGEM`}</Text>
      </Flex>
    </Row>
  )
}
...
export const PrizePoolHistoryRow: React.FC<PrizePoolHistoryRowProps> = ({ totalAmount, ...props }) => {
  ...
  return (
    <Row {...props}>
      <Text bold>{t('Prize Pool')}:</Text>
      <Text bold>{`${getPrizePoolAmountHistory(totalAmount)} EGEM`}</Text>
    </Row>
  )
}
