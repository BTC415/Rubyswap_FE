...
import { CheckmarkCircleIcon, CheckmarkCircleFillIcon, Tag, useTooltip } from '@twinkykms/rubyswap-uikit'
...
const EnteredTag: React.FC<EnteredTagProps> = ({ amount, hasClaimed = false }) => {
  ...
  const { targetRef, tooltipVisible, tooltip } = useTooltip(
    <div style={{ whiteSpace: 'nowrap' }}>{`${formatBnbv2(amount)} EGEM`}</div>,
    { placement: 'bottom' },
  )
