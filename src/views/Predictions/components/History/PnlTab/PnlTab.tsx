...
import { Box, Flex, Heading, Text, Button, Link, OpenNewIcon } from '@twinkykms/rubyswap-uikit'
    ...
    <Box p="16px">
      ...
      <Flex>
        ...
        <Flex flexDirection="column" justifyContent="center" pl="24px">
          ...
          <Text bold fontSize="24px" lineHeight="1" color={netResultIsPositive ? 'success' : 'failure'}>
            {`${netResultIsPositive ? '+' : ''}${formatBnb(netResultAmount)} EGEM`}
          </Text>
      ...
      <Box pl="8px">
        ...
        <Text bold color={avgBnbWonIsPositive ? 'success' : 'failure'}>
          {`${avgBnbWonIsPositive ? '+' : ''}${formatBnb(avgBnbWonPerRound)} EGEM`}
        </Text>
        <Text small color="textSubtle">
        ...
        {hasBestRound && (
          <>
            ...
            <Flex alignItems="flex-end">
              <Text bold color="success">{`+${formatBnb(summary.won.bestRound.payout)} EGEM`}</Text>
        ...
        <Text bold>{`${formatBnb(avgPositionEntered)} EGEM`}</Text>
        <Text small color="textSubtle">
