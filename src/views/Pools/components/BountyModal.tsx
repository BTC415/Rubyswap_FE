...
const BountyModal: React.FC<BountyModalProps> = ({ onDismiss, TooltipComponent }) => {
  ...
  const {
    ...
    totalpendingRubyHarvest,
    fees: { callFee },
  } = useCakeVault()
  const cakePriceBusd = usePriceCakeBusd()
  const callFeeAsDecimal = callFee / 100
  const totalYieldToDisplay = getBalanceNumber(totalpendingRubyHarvest, 18)
  ...
  const handleConfirmClick = async () => {
    ...
    try {
      ...
      if (receipt.status) {
        toastSuccess(t('Bounty collected!'), t('RUBY bounty has been sent to your wallet.'))
        setPendingTx(false)
        onDismiss()
      }
    } catch (error) {
  ...
  return (
    <Modal title={t('Claim Bounty')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <Flex alignItems="center" justifyContent="space-between">
        ...
        <Balance color="textSubtle" value={totalYieldToDisplay} unit=" RUBY" />
      </Flex>
