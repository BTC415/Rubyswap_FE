...
import { Button, Flex, Text, InjectedModalProps } from '@twinkykms/rubyswap-uikit'
...
const StartPage: React.FC<StartPageProps> = ({ goToApprove, goToChange, goToRemove, onDismiss }) => {
  ...
  /**
   * Check if the wallet has the required RUBY allowance to change their profile pic or reactivate
      ...
      <Flex alignItems="center" style={{ height: '48px' }} justifyContent="center">
        <Text as="p" color="failure">
          {!hasMinimumCakeRequired &&
            t('%minimum% RUBY required to change profile pic', { minimum: getFullDisplayBalance(minimumCakeRequired) })}
        </Text>
      </Flex>
      {profile.isActive ? (
        <>
