...
const CakeBnbTokenSection: React.FC<TokenSectionProps> = (props) => {
  return <TokenSection primaryToken={tokens.ruby} secondaryToken={tokens.wegem} {...props} />
}
  ...
  const renderTokenSection = () => {
    ...
    if (account && !hasProfile) {
      if (publicIfoData.status === 'finished') {
        return <Text textAlign="center">{t('Activate RubySwap Profile to take part in next IFO‘s!')}</Text>
      }
      return <Text textAlign="center">{t('You need an active RubySwap Profile to take part in an IFO!')}</Text>
    }
